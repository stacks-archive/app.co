...
const CategoryFilters = ({ categories }) => {
  ...
  const categoryEl = (category, opts) => (
    <Link href={categoryURL(category)} key={category}>
      ...
      >
        <StyledPlatformFilters.PlatformTitle>{category}</StyledPlatformFilters.PlatformTitle>
      </StyledPlatformFilters.Filter>
    </Link>
  )
  ...
  const categoryEls = categoryKeys.slice(0,length).map((category) => (
      <>'       '{categoryEl(category)}'     '</>
    ))
  return (
    <>'     '<StyledPlatformFilters.FilterSubtitle>Show Dapps by Category</StyledPlatformFilters.FilterSubtitle>'     '<StyledPlatformFilters.Filters>
      {categoryEls}
