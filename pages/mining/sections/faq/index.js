...
const FAQSection = ({ faq, apps, ...rest }) => (
  <Section pt={8} bg="blue.dark" {...rest}>
    <Wrapper>
      <Flex width={[1]} flexShrink={0} flexDirection="column">
        <Title maxWidth="100%" pt={8} pb={0}>
          Frequently Asked Questions
        </Title>
        <Box width={1} mt={8} color="white" pl={4} borderLeft={'5px solid'} borderColor="blue.darker">
          {faq.map(({ question }, i) => (
                ...
                <Hover>
                  {({ hovered, bind }) => (
                    <Flex
                      ...
                      {...bind}
                    >
                      <Type fontSize={2} fontWeight="500" color={hovered ? 'white' : 'blue.mid'}>
                        {question}
                      </Type>
                    </Flex>
                  )}
        ...
        <Box mt={9} width={80} height={'1px'} bg={'blue.accent'} />
        <Box width={1} pt={5} color="white">
          {faq.map(({ question, answer }, i) => (
            <React.Fragment key={i}>
              <Box id={slugify(question)} pt={7}>
                ...
                <Box lineHeight={1.65} color="blue.mid">
                  <Type maxWidth={900} dangerouslySetInnerHTML={{ __html: answer }} />
                </Box>
                <Box mt={6} width={80} height={'1px'} bg={'blue.accent'} />
              </Box>
            </React.Fragment>
          ))}
        </Box>
